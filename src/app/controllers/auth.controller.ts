...
async function login(req: Request, res: Response): Promise<Response> {
  try {
    ...
    const filter: mongoose.FilterQuery<User> = {
      $or: [{ email: loginType }, { phoneNumber: loginType }],
    };
    ...
    await UserService.updateOneUserById(user._id, {
...
async function refreshToken(req: Request, res: Response): Promise<Response> {
  try {
    ...
    const user: User | null = await UserService.getOneUser({
      _id: existsSession.userId,
    });
...
async function logout(req: Request, res: Response): Promise<Response> {
  try {
    ...
    await UserService.updateOneUserById(user._id, {
      $pull: { sessions: existsSession._id },
    });
